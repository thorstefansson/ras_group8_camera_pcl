...
public:
    ...
    vision(ros::NodeHandle& nodeHandle): nodeHandle_(nodeHandle)
    {
        ...
        //initial record cloud
         ...
         //if running on ras 28:
         resultFile.open("/home/ras28/catkin_ws/src/ras_group8/ras_group8_camera1/classifiers/colors_itr1e4_1e-5_gamma001_newer.dat");
         ...
         //if running on ras28
         std::string kdtree_idx_file_name = "/home/ras28/catkin_ws/src/ras_group8/ras_group8_camera_pcl/kdtree.idx";
         std::string training_data_h5_file_name = "/home/ras28/catkin_ws/src/ras_group8/ras_group8_camera_pcl/training_data.h5";
         std::string training_data_list_file_name = "/home/ras28/catkin_ws/src/ras_group8/ras_group8_camera_pcl/training_data.list";
        ...
        sub = nodeHandle_.subscribe<sensor_msgs::PointCloud2> ("/camera/depth_registered/points", 1, &vision::sensor_cb,this);

        sub_barcode = nodeHandle_.subscribe<std_msgs::String>("/barcode",1,&vision::QR_cb,this);
...
*/
    ...
    void QR_cb(const std_msgs::String::ConstPtr& msg){
        trap_secs =ros::Time::now().toSec();
        //is_trap = 1;
        ROS_INFO("in QR code call back");
...
*/
        ...
        //pcl::PCLPointCloud2ConstPtr cloudPtr(cloud_outliers);
        pcl::PointCloud<pcl::PointXYZHSV>::Ptr cloudPtr = cloud_outliers.makeShared();
        pcl::PointCloud<pcl::PointXYZHSV> cloud_filtered_lower;
        pcl::PointCloud<pcl::PointXYZHSV> cloud_filtered_upper;
        ...
        // Segment cloud into lower and upper cloud:
        ...
        lower_cloud_no_floor = cloud_filtered_lower.makeShared();
        pcl::PointCloud<pcl::PointXYZHSV>::Ptr upper_HSV_cloudPtr = cloud_filtered_upper.makeShared();
      ...
      //REMOVE THE SAME PLANE FROM LOWER CLOUD!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       // cout <<"lower cloud without wall:" << cloud_no_wall_outliers->width * cloud_no_wall_outliers->height << endl;
        ...
        //   cloud_outliers = cloud_outliers;
          ...
          tree->setInputCloud (lower_cloud_no_floor);
        ...
        //HSV:
          ...
          ec.setInputCloud (lower_cloud_no_floor); //here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
          {
              ...
              for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); pit++)
                  cloud_cluster->points.push_back (lower_cloud_no_floor->points[*pit]);//here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          /*
            ...
            */
            ...
            if((float)number_of_color_votes/size < 0.2){
                ...
                cout << "size: " << size << endl;
                (color_probs[order])[7] = 1;

                if(number_of_trap_votes/size > 0.5) (color_probs2[order])[8] = 1;
                else (color_probs2[order])[7] = 1;
            }

            //if barcode has recently been detected:
            /*
            cout<<"trap secs"<<trap_secs <<endl;
            cout<<"current secs"<<ros::Time::now().toSec() <<endl;

            cout <<"condition: " << ros::Time::now().toSec()-trap_secs << endl;
*/
            if(ros::Time::now().toSec()-trap_secs < 4){

                //cout <<"in if condition"<< endl;
                (color_probs2[order])[8] = 1.1;
            }
            //cout <<"trap prob: " << (color_probs2[order])[8] << endl;
    ...
    void compare(const pcl::PointCloud<pcl::VFHSignature308> &point, int inmum)
    {
        ...
        //if running on ras 28:
        flann::Index<flann::ChiSquareDistance<float> > index (data, flann::SavedIndexParams ("/home/ras28/catkin_ws/src/ras_group8/ras_group8_camera_pcl/kdtree.idx"));
